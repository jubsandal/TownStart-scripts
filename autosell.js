...
(function() {
	...
	{ item: 'Energy',   keepAmt: 0, sellMin: 10 },
    ]

    async function getGame() {
        console.log("Awaiting game instance");
        while (typeof Game == 'undefined' || (Game && Game.gameData == null)) {
            console.log("Cannot reach game instance");
            await new Promise(r => setTimeout(r, 500));
        }
	console.log("Game instance resolved")
    }

    let autoSellLoaded = 0;
    new MutationObserver(async function(mutations) {
        if (autoSellLoaded == 0) {
            console.log('Auto cesll active');
            autoSellLoaded = 1;
            await getGame();
            ActivateAutoSell();
        } else {
	    console.log("Cannot active auto sell")
	}
    ...
    async function ActivateAutoSell() {
        ...
        autoSellContent.style.cssText = 'padding-left: 10px; padding-right: 10px';
